...
def extend_data(acc, loss, epochs=100):
    if len(acc) >= epochs:
        return acc[:epochs], loss[:epochs]
...
# deit_small (low rank qk)

acc0, loss0 =  (get_data('./exp_hr/svd/deit_small_5e-4_5e-6_100_mix_head_fc_qk/log.txt'))
acc1, loss1 =  (get_data('./exp_hr/svd/deit_small_1e-4_5e-6_100_mix_head_fc_qk/log.txt'))
acc2, loss2 =  (get_data('./exp_hr/svd/deit_small_5e-5_5e-6_100_mix_head_fc_qk/log.txt'))
acc3, loss3 =  (get_data('./exp_hr/svd/deit_small_1e-5_5e-6_100_mix_head_fc_qk/log.txt'))
acc4, loss4 =  (get_data('./exp_hr/svd/deit_small_5e-6_100_mix_head_fc_qk/log.txt'))
...
recon_loss = get_recon_loss('./exp_hr/svd/deit_small_1e-4_5e-6_100_mix_head_fc_qk/log_detail.txt')
...
x = range(100)
ax1.plot(x, acc0, label = "lr=5e-4_5e-6", c=colors[0])
ax1.plot(x, acc1, label = "lr=1e-4_5e-6", c=colors[1])
ax1.plot(x, acc2, label = "lr=5e-5_5e-6", c=colors[2])
ax1.plot(x, acc3, label = "lr=1e-5_5e-6", c=colors[3])
ax1.plot(x, acc4, label = "lr=5e-6_5e-6", c=colors[4])
ax1.plot(x[:100], [79.9] * 100, '--', label = 'unpruned', c=colors[5])
ax1.title.set_text('Accuracy')
ax2.plot(x, loss0, label = "lr=5e-4_5e-6", c=colors[0])
ax2.plot(x, loss1, label = "lr=1e-4_5e-6", c=colors[1])
ax2.plot(x, loss2, label = "lr=5e-5_5e-6", c=colors[2])
ax2.plot(x, loss3, label = "lr=1e-5_5e-6", c=colors[3])
ax2.plot(x, loss4, label = "lr=5e-6_5e-6", c=colors[4])
ax2.title.set_text('Test loss')
ax3.plot([i+1 for i in range(len(recon_loss))], recon_loss, label='loss', c=colors[1])
...
f.tight_layout()
plt.savefig("./exp_hr/mix_head_fc_qk_deit_small.png")


# deit_small (low rank qk + sparse)

acc0, loss0 =  (get_data('./exp_hr/info_0.8/deit_small_lowrank_qk_1e-5_100/log.txt'))

acc0, loss0 = extend_data(acc0, loss0)

f, (ax1, ax2) = plt.subplots(1, 2, sharex=True)
x = range(100)
ax1.plot(x, acc0, label = "lr=1e-5")
ax1.plot(x[:100], [79.9] * 100, '--', label = 'unpruned')
ax1.title.set_text('Accuracy')
ax2.plot(x, loss0, label = "lr=1e-5")
ax2.title.set_text('Test loss')
plt.legend()
f.tight_layout()
plt.savefig("./exp_hr/info_0.8_deit_small_mix_head_qk.png")