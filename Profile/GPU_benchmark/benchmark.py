    ...
    if enable_op_profiling:
        ...
        with torch.profiler.profile(
            schedule=torch.profiler.schedule(
                ...
                active=num_iters,
                repeat=1),
            on_trace_ready=torch.profiler.tensorboard_trace_handler(os.path.join("benchmark_logs/{}".format(model_name))),
            with_stack=False, # with_stack=True incurs an additional overhead, and is better suited for investigating code. Remember to remove it if you are benchmarking performance.
            profile_memory=True,
            with_flops=True
        ) as profiler:
...
def locate_arch_T(model, model_config, init_batch_size, warmup_iters=3, num_iters=30, enable_op_profiling=False, return_bs=False):
    assert (init_batch_size & (init_batch_size-1) == 0) and init_batch_size != 0, ("batch_size_step should be power-of-2")
    batch_size = init_batch_size


    # imgs_per_sec = compute_throughput(model, model_config, batch_size, warmup_iters=warmup_iters, num_iters=num_iters, enable_op_profiling=enable_op_profiling)
