...
class FeedForward(nn.Module):
    def __init__(self, dim, mult = 4, dropout = 0., activation = None, glu = False, tflite=False):
        super().__init__()
        activation = default(activation, GELU_ if tflite else nn.GELU)
...
class LinformerSelfAttention(nn.Module):
    ...
    def forward(self, x, context = None, **kwargs):
        ...
        keys = keys.transpose(-1, -2) # 'bnd-> bdn'
        keys = torch.matmul(keys, kv_projs[0]) # 'bdn,nk-> bdk'
        keys = keys.transpose(-1, -2) # 'bdk-> bkd'
        values = values.transpose(-1, -2) # 'bnd-> bdn'
        values = torch.matmul(values, kv_projs[1]) # 'bdn,nk-> bdk'
        values = values.transpose(-1, -2) # 'bdk-> bkd'
        ...
        dots = torch.matmul(queries, keys.transpose(-1, -2)) * (d_h ** -0.5)
...
class Linformer(nn.Module):
    def __init__(self, dim, seq_len, depth, k = 256, heads = 8, dim_head = None, one_kv_head = False, share_kv = False, reversible = False, dropout = 0., tflite=False):
        super().__init__()
        ...
        for _ in range(depth):
            ...
            ff = FeedForward(dim, dropout = dropout, tflite=tflite)
