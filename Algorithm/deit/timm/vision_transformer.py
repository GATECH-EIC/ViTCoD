...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
        ...
        elif self.svd_type == 'mix_head_fc_q':
            hidden = num_heads // 2
            self.encoder_q = nn.Linear(num_heads, hidden)
            self.decoder_q = nn.Linear(hidden, num_heads)
        elif self.svd_type == 'mix_head_fc_k':
            self.encoder_k = nn.Linear(num_heads, hidden)
            self.decoder_k = nn.Linear(hidden, num_heads)
        else:
    ...
    def forward(self, x, evaluate=False):
        ...
        if self.svd_type is None:
            pass
        elif self.svd_type == 'single_head':

            q_shape = q.shape
            k_shape = k.shape

            q = q.contiguous().view(-1, q.shape[2], q.shape[3]) # (bh) x n x d
            k = k.contiguous().view(-1, k.shape[2], k.shape[3]) # (bh) x n x d

            pre_q = q
            pre_k = k

            # q_u, q_sigma, q_v = torch.linalg.svd(q, full_matrices=False)
            # k_u, k_sigma, k_v = torch.linalg.svd(k, full_matrices=False)

            # q_u, q_sigma, q_v = torch.svd(q, some=True)
            # k_u, k_sigma, k_v = torch.svd(k, some=True)

            q_u, q_sigma, q_v = torch.svd_lowrank(q, q=100)
            k_u, k_sigma, k_v = torch.svd_lowrank(k, q=100)

            # print('q shape: {}'.format(q.shape))
            # print('u, sigma, v shapes: {}, {}, {}'.format(q_u.shape, q_sigma.shape, q_v.shape))

            # reconstruction
            q = torch.matmul(torch.matmul(q_u, torch.diag_embed(q_sigma)), q_v.transpose(-2, -1))
            k = torch.matmul(torch.matmul(k_u, torch.diag_embed(k_sigma)), k_v.transpose(-2, -1))
            ...
            # q = q_u @ torch.diag(q_sigma) @ q_v
            # k = k_u @ torch.diag(k_sigma) @ k_v

            q = q.view(q_shape)
            k = k.view(k_shape)

        elif self.svd_type == 'mix_head':

            print('evaluate: {}'.format(evaluate))

            if evaluate is False: # training
                if torch.rand(1) < 0.03:
                    flag = True
                else:
                    flag = False
            else: # testing
            ...
            if flag is True:
                ...
                # q = q.contiguous().view(q.shape[0], q.shape[2], -1) # b x n x (hd)
                # k = k.contiguous().view(k.shape[0], k.shape[2], -1) # b x n x (hd)
                ...
                # q_u, q_sigma, q_v = torch.svd_lowrank(q, q=6)
                # k_u, k_sigma, k_v = torch.svd_lowrank(k, q=6)

                q_u, q_sigma, q_v = torch.pca_lowrank(q, q=6, niter=1)
                k_u, k_sigma, k_v = torch.pca_lowrank(k, q=6, niter=1)

                # print('q shape: {}'.format(q.shape))
                # print('u, sigma, v shapes: {}, {}, {}'.format(q_u.shape, q_sigma.shape, q_v.shape))

                # reconstruction
                q_v = q_v.transpose(-2, -1)
                q = torch.matmul(torch.matmul(q_u, torch.diag_embed(q_sigma)), q_v)
                k = torch.matmul(torch.matmul(k_u, torch.diag_embed(k_sigma)), k_v.transpose(-2, -1))

                # print('reconstruction loss for q: {}'.format(torch.dist(q, pre_q)))
                # print('reconstruction loss for k: {}'.format(torch.dist(k, pre_k)))
        ...
        elif self.svd_type == 'mix_head_fc_qk':
            ...
            self.recon_loss = torch.dist(k, pre_k)
            ...
            self.recon_loss = torch.dist(q, pre_q)
            ...
            print('No such svd_type supported.')
            exit()

        # attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = self.cal_attn(q, k.transpose(-2, -1)) * self.scale
        ...
        # print (attn.shaspe)
        # x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        x = self.cal_out(attn, v).transpose(1, 2).reshape(B, N, C)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
################################ Masked Attention End ########################
...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = x + self.drop_path(self.attn(self.norm1(x), evaluate=self.evaluate))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        self.recon_loss = self.attn.recon_loss
        return x
################################ Masked Block End ########################
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        print('need weight: ', need_weight)
        print('svd type: ', svd_type)
        ...
        if mask_path is not None and mask_path != '':
            attn_mask = self._generate_patterns(mask_path, img_size, patch_size,
                                                num_heads, depth, embed_dim)
            
        else:
            attn_mask = [None] * depth
        self.blocks = nn.Sequential(*[
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,
                need_weight=need_weight, attn_mask=attn_mask[i], svd_type=svd_type)
            for i in range(depth)])
        self.norm = norm_layer(embed_dim)
    ...
    def _generate_patterns(self, mask_path,
        ...
        with torch.no_grad():
            ...
            if utils.is_main_process():
                # print('#'*20, 'Print generated patterns', '#'*20)
                # for seed in range(layers):
                #     print('%02d' % (seed)*30, pattern_mask[seed][0,0,1,1:])
                # print('#'*50)

                # Print the ratio of the remaining elements.
                mask_utils.print_pattern_ratio(pattern_mask)

                # Save reduced number of FLOPs as a member variable.
    ...
    def forward_features(self, x, evaluate=False):
        x = self.patch_embed(x)
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        if self.dist_token is None:
            x = torch.cat((cls_token, x), dim=1)
        ...
        for i in range(len(self.blocks)):
            self.blocks[i].evaluate = evaluate
        x = self.blocks(x)
        x = self.norm(x)
        if self.dist_token is None:
            return self.pre_logits(x[:, 0])
    ...
    def forward(self, x, evaluate=False):
        x = self.forward_features(x, evaluate=evaluate)
        if self.head_dist is not None:
            x, x_dist = self.head(x[0]), self.head_dist(x[1])  # x must be a tuple
            if self.training and not torch.jit.is_scripting():
                # during inference, return the average of both classifier predictions
        ...
        if self.svd_type is not None:
            return x, recon_loss
        else:
            return x, 0
################################ Masked VisionTransformer End ########################
